...
class PnP_solve:
    def __init__(self,rect_width=210.0,rect_length=305.0,focal_length=600.0,center_point=(224.0,224.0)):
        """
        初始化 3D 世界物理坐标系与相机参数
        ...
        self.camera_matrix_inv = np.linalg.inv(self.camera_matrix) #内参逆矩阵

        # 加载标定参数 失败时保留上面的默认相机参数
        self.load_camera_calib("camera_calib.json")

    def load_camera_calib(self, json_path="camera_calib.json"):
        """
        加载相机内参矩阵与 OpenCV 畸变参数。
        /root/camera_calib.json 板端标定程序生成
        """
        candidate = json_path

        if candidate is None:
            print("未找到相机标定文件，使用默认内参")
    ...
    def solve(self,image_point):
        ...
        solve_flag = cv2.SOLVEPNP_IPPE if hasattr(cv2, "SOLVEPNP_IPPE") else cv2.SOLVEPNP_ITERATIVE
        success,rvec,tvec = cv2.solvePnP(self.object_points,pts_img,self.camera_matrix,\
                                         self.dist,flags=solve_flag)
    ...
    def point_board(self,point_x,point_y,rvec,tvec):
        ...
        d_cam = self.camera_matrix_inv @ pixel_vec
